    ...
    <g id="Brand-Logo" transform="translate(-27.000000, -15.000000)">
      <g id="Icon" transform="translate(27.000000, 15.000000)">
        ...
        <g id="Triangle" transform="translate(19.000000, 11.000000) rotate(-300.000000) translate(-19.000000, -11.000000) ">
          ...
          <span class="app-brand-text demo menu-text fw-bold">SISTEM ABSENSI</span>
        </a>
    ...
    <div class="card-body">
      <div class="d-flex align-items-start justify-content-between">
        <div class="content-left">
          ...
          <div class="d-flex align-items-end mt-2">
            <?php
              date_default_timezone_set('Asia/Jakarta'); // Atur zona waktu menjadi WIB
              echo '<h4 class="mb-0 me-2">' . date('H:i:s') . '</h4>'; // Tampilkan waktu saat ini
            ?>
          </div>
          <p class="mb-0">Waktu Indonesia Barat</p>
        </div>
        <div class="avatar">
...
// Query untuk mengambil data absensi dengan JOIN ke tabel akun dan kartu
$sql = "SELECT absen.id_absen, akun.nama AS nama_pengguna, absen.jam_masuk, absen.jam_pulang, absen.foto_masuk, absen.foto_pulang
        FROM absen
        JOIN akun ON akun.id_user = absen.id_user
        JOIN kartu ON akun.id_kartu = kartu.id_kartu";
$result = $conn->query($sql);

if ($result && $result->num_rows > 0) {
    // Menampilkan data absensi
    echo '<div class="card-datatable table-responsive">';
    echo '<table class="datatables-users table border-top">';
    echo '<thead>';
    echo '<tr>';
    echo '<th>ID Absen</th>'; // Ubah sesuai dengan nama kolom yang benar
    echo '<th>Nama Pengguna</th>';
    echo '<th>Jam Masuk</th>';
    echo '<th>Jam Pulang</th>';
    echo '<th>Foto Masuk</th>';
    echo '<th>Foto Pulang</th>';
    echo '</tr>';
    echo '</thead>';
    echo '<tbody>';
    ...
    while ($row = $result->fetch_assoc()) {
        echo '<tr>';
        echo '<td>' . $row['id_absen'] . '</td>'; // Ubah sesuai dengan nama kolom yang benar
        echo '<td>' . $row['nama_pengguna'] . '</td>';
        echo '<td>' . $row['jam_masuk'] . '</td>'; // Tambahkan kolom jam_masuk jika tersedia dalam tabel absen
        echo '<td>' . $row['jam_pulang'] . '</td>'; // Tambahkan kolom jam_pulang jika tersedia dalam tabel absen
        echo '<td> <a href="../uploads/image_' . $row['foto_masuk'] . '.jpg" target="_blank"><button>Lihat Foto Masuk</button></a> </td>';
        if ($row['foto_pulang'] !== null) {
          echo '<td> <a href="../uploads/image_' . $row['foto_pulang'] . '.jpg" target="_blank"><button>Lihat Foto Pulang</button></a></td>';
      }
      echo '</tr>';
    }
